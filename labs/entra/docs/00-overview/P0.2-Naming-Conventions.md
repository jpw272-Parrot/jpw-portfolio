...
## Purpose
Define consistent naming and namespace rules for identities, groups, devices, and service accounts to support hybrid Entra ID + AD, and to reduce operational and audit risk. JPW is the lab namespace; MAH/ATS are the simulated business entities.
...
### OU naming & admin OU intent (Draft — refined in Phase 1)

**Purpose:** Define a minimal OU pattern for tiered administration. Final OU/GPO layout will be implemented and validated in Phase 1 (P1.3).

**Tiered Admin OU pattern (corp + ats forests)**
- `OU=Admin,DC=<forest>,DC=jpw,DC=lab`
  - `OU=Tier0,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
    - `OU=Accounts,OU=Tier0,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
    - `OU=Groups,OU=Tier0,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
  - `OU=Tier1,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
    - `OU=Accounts,OU=Tier1,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
    - `OU=Groups,OU=Tier1,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
  - `OU=Tier2,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
    - `OU=Accounts,OU=Tier2,OU=Admin,DC=<forest>,DC=jpw,DC=lab`
    - `OU=Groups,OU=Tier2,OU=Admin,DC=<forest>,DC=jpw,DC=lab`

**Rule of placement**
- Privileged admin accounts (`t0-`, `t1-`, `t2-`) live under the matching `OU=Accounts,OU=TierX,OU=Admin,...`
- Break-glass accounts (`bg-`) are **cloud-only** (not placed in AD OUs)

**Evidence plan**
- OU tree export and GPO scaffolding evidence captured in Phase 1 (P1.3).
